...
const products = [
  {
    ...
    image: "https://test-task-mcg.vercel.app/static/Images/Landing/ClothImage.png",
    rating: 4.5,
    price: 30,
  },
  {
      ...
      <div className="gap-[55px] max-w-[1240px] flex flex-col items-start justify-center w-full">
        ...
        <div className="gap-5 w-full grid grid-cols-[repeat(auto-fill,minmax(287px,1fr))] place-items-center">
          {products.slice(0, visibleCount).map((item) => (
            <div
              key={item.id}
              className="rounded-[20px] flex flex-col gap-2 max-w-[300px] transition-all duration-500 ease-in-out transform "
            >
              <img
                src={item.image}
                alt={item.title}
        ...
        {visibleCount < products.length && (
          <div className="flex flex-row items-center justify-center w-full">
            <button
              ...
              className="text-center rounded-[62px] border-2 border-[#F0EEED] py-4 px-14 cursor-pointer "
            >
              View All
            </button>
          </div>
